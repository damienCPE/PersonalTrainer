...
public class SearchExerciseServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		paramFilter = request.getParameter("filter");
		// System.out.println(paramFilter);
		if (paramFilter != null) {
			exFilter = new FilterPredicate("title", FilterOperator.EQUAL,
					paramFilter);
		} else {
			paramFilter = request.getParameter("trainingId");
			// System.out.println("id = " + paramFilter);
			exFilter = new FilterPredicate("trainingId", FilterOperator.EQUAL,
					Long.parseLong(paramFilter));
		}

		// Use class Query to assemble a query
		Query q = new Query("Exercice").setFilter(exFilter);
		...
		for (Entity result : pq.asIterable()) {
			...
			// System.out.println(result.getProperty("title") + " "
			...
			innerMap.put("trainingId",
					String.valueOf(result.getProperty("trainingId")));
			innerMap.put("time",
					String.valueOf(result.getProperty("time")));
